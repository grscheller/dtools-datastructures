...
class TestQueueTypes:
    ...
    #   assert len(lq) == 0
        ...
        lq2: LIFOQueue[Optional[int]] = LIFOQueue(sentinel=nothing)
        ...
        bar42 = (42, 1, 2, 42, 3)
        lq3 = LIFOQueue(*barNone, sentinel=nothing)
        lq4 = LIFOQueue(*map(is42, bar42), sentinel=nothing)
        assert lq3 == lq4
    ...
    #   assert dq1 == dq2
        ...
        lq1 = LIFOQueue(3, 'Forty-Two', l1, 1, sentinel=nothing)
        lq2 = LIFOQueue(3, 'Forty-Two', 2, sentinel=nothing)
        assert lq1.pop() == 1
        peak = lq1.peak()
        assert peak == l1
        ...
        lq3: LIFOQueue[string, nothing] = LIFOQueue(*map(lambda i: str(i), range(43)), sentinel=nothing)
        lq4: LIFOQueue[string, nothing] = LIFOQueue(*range(-1, 39), 41, 40, 39, sentinel=nothing)
